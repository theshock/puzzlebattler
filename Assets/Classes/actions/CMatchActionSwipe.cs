...
public class CMatchActionSwipe : CMatchBaseAction
{
			...
			if(matches.Count > 0)
			{
				foreach(ArrayList match in matches)
				{
					for(int j = 0; j < match.Count; j++)
					{
						...
						{
							IMatchAction matchAction = mActionManager.createAction(EMatchAction.eMatchAction);
